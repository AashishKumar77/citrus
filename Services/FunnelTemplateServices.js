...
var table_name = Models.FunnelTemplate;

//Get Users from DB

var getData = function (criteria, projection, options, callback) {
// var getData = function (criteria, options, callback) {
    //console.log(criteria,"criteria")
    table_name.find(criteria).sort( { emailTimeInterval : 1 } ).exec(function(err,result){
...
var getAllFunnels = function (criteria, callback) {
    table_name.find(criteria).sort( { emailTimeInterval : 1 } ).exec( function(err,result){ ////console.log("criteria_err",err);
        if(err) {
            // if(err.name=="CastError") return callback(responses.INVALID_USER_ID);
            return callback(err);
        }
