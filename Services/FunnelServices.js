...
var table_name = Models.FUNNEL_MODAL;
...
var addUnsubscribeUser = function (criteria, dataToSet, options, callback) { ////console.log("here+++++++xxxx++++",criteria,dataToSet);
    table_name.findOneAndUpdate(criteria, { $push : { unsubscribedUsers : dataToSet } }, options, function(err,result){
        if(err) { ////console.log("==========updateUser===========",err);
            if(err.name=="CastError") return callback(responses.INVALID_POST_ID);
            return callback(err);
        }
...
var checkData = function (criteria, callback) { ////console.log("here+++++++xxxx++++",criteria,dataToSet);
...
module.exports = {
    ...
    addUnsubscribeUser  : addUnsubscribeUser,
    checkData           : checkData
};
