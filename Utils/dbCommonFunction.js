...
exports.getDataPopulateOneLevel = function (model,query,projectionQuery,options,populateModel,callback) {
    // //console.log("populateModel",model,projectionQuery,"=====",populateModel);

    // //console.log(' SOMETIMES......    ',JSON.stringify(query));

    model.find(query, projectionQuery, options).populate(populateModel).exec(function (err, docs){
        if (err)  return callback(err);

        // //console.log('Testimonials found ',docs);
